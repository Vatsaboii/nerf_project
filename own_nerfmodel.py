...
def train(nerf_model, optimizer, scheduler, data_loader, device='cpu', hn=0, hf=1, nb_epochs=int(1e5),
    ...
    for epoch in tqdm(range(nb_epochs)):
        ...
        # Log training loss
        logger.info(f"Epoch [{epoch+1}/{nb_epochs}], Loss: {np.mean(training_loss[-len(data_loader):]):.4f}")

        scheduler.step()

        # Run test function for some images
        if epoch % 10 == 0:
            for img_index in range(5):  # Render only a few images for monitoring
